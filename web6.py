...
def home():
    ...
    if request.args.get("admin") == "0398daed503b4ca7a3833bafaa959611":
        admin = "true"
    return render_template('smartlab.html', comments=db.smartcomments.find(sort=[( '_id', DESCENDING )]), admin=admin)

@app.route("/insert_comment", methods=["POST"])
def insert_comment():
    print(request.form)
    db.smartcomments.insert_one({ "date" : datetime.now().strftime("%d/%m/%Y %H:%M:%S"), "id" : uuid.uuid4().hex, "name" : request.form.get('name'), "comment" : request.form.get('comment') })
    return redirect("/")

@app.route("/delete_comment")
def delete_comment():
    comment_id = request.args.get('comment')
    db.smartcomments.delete_one({"id" : comment_id})
    return redirect("/")
...
def alter_comment():
    ...
    comment_id = request.args.get('comment')
    db.smartcomments.update_one({"id" : comment_id}, {"$set" : {"comment" : request.form.get('comment')}})
